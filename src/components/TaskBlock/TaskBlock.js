...
function TaskBlock({ dayName, children, border, position, tasks, onTaskClick }) {
  return (
    <div className={`col-span-1 px-[20px] border-r-2 border-[#373942] flex flex-col h-full ${border} ${position} ${styles.addButtonContainer}`}>
        ...
        <div>
          {tasks.map((task, index) => (
            <div key={index} className='w-full p-2 mb-2 bg-[#8367C7] text-white rounded cursor-pointer' onClick={() => onTaskClick(task)}>
              <p>{task.tas_name} 
